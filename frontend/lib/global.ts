...
function updateCurrentSessionStartTime(newTimestamp: string) {
    console.log("updating current session start time to: ", newTimestamp);
    state.currentSessionStartTime = newTimestamp;
    return state.currentSessionStartTime;
}

export { state, emitter, QUERY_LIMIT, MessageData, GlobalState, ThreadChangePayload, MenuChangePayload, addMessage, addPreview, updatePageNumber, ThreadPreviewData, updateMessageStatus, MessageSuccessPayload, updateCurrentSessionStartTime }
